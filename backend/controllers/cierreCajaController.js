  ...
  try {
    ...
    if (req.body?.fecha) {
      ...
      if (diasAtras < 0 || diasAtras > 6) {
        return res.status(400).json({ message: "Solo se puede cerrar la caja del día actual o de los últimos 6 días" });
      }
    }
...
export const abrirCaja = async (req, res) => {
  try {
    ...
    if (fecha !== hoy) {
      ...
      if (diasAtras > 6) {
        return res.status(400).json({ message: "Solo se pueden abrir las cajas de los últimos 6 días" });
      }
    }
